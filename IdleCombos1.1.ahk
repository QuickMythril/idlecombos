#include JSON.ahk
;Added in 1.1:
;-Info for "Slow and Steady" & "Veterans of Avernus" blessings.
;-Displays Feats or Shinies from opening chests.
;-Link to Discord Server
;-Changed from the default AHK icon.

;Fixed in 1.1:
;-Prompt for install directory when choosing not to scan default location.
;-(previously this only prompted on failed scan attempt)
;-Log now autoscrolls to bottom!
;-(except on first loading)
;-Fixed timestamp date issues.
;-No repeat save dialogs when opening more than 99 chests.
	...
	while (count > 0) {
		...
		for k, v in chestresults.loot_details {
			if (v.unlock_hero_feat) {
				newfeats += v.unlock_hero_feat "`n"
			}
			if (v.gilded) {
				newshinies += ChampFromID(v.hero_id) " (Slot " v.slot_id ")`n"
			}
		}
...
Discord_Clicked:
{
	Run, % "https://discord.com/invite/6tZuC3H"
	return
}
