#include JSON.ahk
;Added in 1.0.3:
;-Support for partial hour offset timezones
;-Displays current data timestamp
;-Continous opening/buying of more than 99 chests
;-Better error checking in server responses

;Fixed in 1.0.3:
;-Achievement info listed in 1.0.2 did not work at all
;-Fixed TGP date rollover display issue
...
global AchievementInfo := "This page intentionally left blank.`n"
;Inventory globals
...
class MyGui {
	...
	__New()
	{
		...
		Menu, FileSubmenu, Add, &Reload, Reload_Clicked
		Menu, FileSubmenu, Add, E&xit, Exit_Clicked
		...
		Menu, HelpSubmenu, Add, &Open Support Ticket, Open_Ticket
...
Open_Chests(chestid) {
	...
	{
		case 1: {
			InputBox, count, Buying Chests, % "How many Silver Chests?`n(Max: " CurrentSilvers ")", , 200, 180
			if ErrorLevel
				return
		...
		case 2: {
			InputBox, count, Buying Chests, % "How many Gold Chests?`n(Max: " CurrentGolds ")", , 200, 180
			if ErrorLevel
				return
...
CheckAchievements() {
	...
	AchievementInfo := "Locked Achievement Details`n" todoasharra todogromma todokrond todoregis
	if (AchievementInfo == "Locked Achievement Details`n") {
		AchievementInfo := "This page intentionally left blank.`n"
	}
}
